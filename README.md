...
| `tbaEventCode` | string | TBA event code (e.g. `"2026njski"`); used by `/api/get-tba-rank`, the OPR Rankings sidebar, and the on-form Statbotics `MatchPrediction` card | None |
...
| `prescout` | object | Config-driven prescout form at `/scout-leads/prescout/`. Contains `sections[]` with structured fields for per-team data entry. See [Config-Driven Prescout Form](#config-driven-prescout-form). | None |
...
- Prescout spreadsheet table: `prescout_<gameName>`
- Prescout form table: `prescoutform_<gameName>`
- Photos table: `photos_<gameName>`
- Field images table: `fieldimages_<gameName>`
...
The prescout feature has two complementary data sources:

1. **Config-driven prescout form** — scouts fill out a structured form at `/scout-leads/prescout/` based on the `prescout` top-level key in the game config. Data is stored per team in `prescoutform_<gameName>`.
2. **Spreadsheet bulk upload** — admins or scouts upload a `.xlsx` file at `/scout-leads/prescout/upload`. Data is stored per team in `prescout_<gameName>`.

On `/team-view` and `/compare`, both sources are fetched in parallel and form data takes priority over spreadsheet data. The `PrescoutSection` component renders the same `[{field, value}]` array format regardless of source.

### Database Tables

All tables are created automatically when a game config is activated (inside `createGame()` in `src/lib/game-config.js`) and dropped when the game is deleted. Table names are generated by helper functions in `src/lib/schema-generator.js`.

| Table pattern | Key columns | Notes |
|---------------|-------------|-------|
| `prescout_<gameName>` | `team_number UNIQUE`, `data` (JSONB), `uploaded_at` | Spreadsheet upload data. One row per team; upsert on re-upload. No `game_name` column — isolation is by table name. |
| `prescoutform_<gameName>` | `team_number UNIQUE`, `data` (JSONB), `submitted_by`, `submitted_at`, `updated_at` | Config-driven prescout form data. One row per team; upsert on re-submit. `submitted_by` is `"scoutName (scoutTeam)"`. Table name from `sanitizePrescoutFormTableName()`. |
...
### Config-Driven Prescout Form

The `prescout` top-level key in the game config JSON defines a structured form that any authenticated scout can fill out at `/scout-leads/prescout/`. This replaces the admin-only spreadsheet workflow for per-team structured data entry.
    ...
    "sections": [
      {
        "id": "robot_info",
        "header": "Robot Info",
        "description": "Ask the team directly",
        "fields": [
          {
            "type": "singleSelect",
            "name": "drive_type",
            "label": "Drive Type",
            "description": "Don't ask, just observe",
            "options": [
              { "value": 0, "label": "Swerve" },
              { "value": 1, "label": "Tank" }
            ]
          },
          {
...
#### `prescout` Top-Level Key

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `sections` | array | Yes | Array of section objects defining the form layout |
...
| `header` | string | Yes | Section heading displayed above the fields |
| `description` | string | No | Optional explanatory text shown below the header |
| `fields` | array | Yes | Array of field objects |

#### Prescout Field Properties

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `name` | string | Yes | Unique internal key for the field (must be unique across all prescout sections) |
| `type` | string | Yes | Field type: `singleSelect`, `comment`, `starRating`, `checkbox`, or `multiSelect` |
| `label` | string | Yes | Display label shown above the field; also used as the stored key in `[{field, value}]` pairs |
| `description` | string | No | Optional help text shown below the label |
| `options` | array | Required for `singleSelect` | Array of `{ value: integer, label: string }` option objects |
| `hasOther` | boolean | No | For `singleSelect` only. When `true`, an **"Other"** tile is appended after the configured options. Selecting it reveals a free-text input. The typed string is stored as the display value in the `[{field, value}]` array instead of an option label. |
| `max` | integer ≥ 2 | No | For `starRating`: number of stars to render (default: 5 in prescout form) |
| `showWhen` | object | No | Conditional visibility rule. The field is hidden unless the condition is met. Hidden fields are excluded from form submission and their values are cleared automatically when the controlling field changes. See [`showWhen` Syntax](#prescout-showwhen-syntax) below. |

**Validation:** `config-validator.js` warns if `id` or `name` is missing, if `type` is not one of the valid values, if `singleSelect` fields lack `options`, if `starRating` has `max < 2`, if `hasOther` is present but not a boolean, if `showWhen.field` is missing or references a field not found among preceding fields in the same config, or if field names are duplicated.

#### `showWhen` Syntax (Prescout Fields) {#prescout-showwhen-syntax}

```json
{ "showWhen": { "field": "<other-field-name>", "equals": <value> } }
{ "showWhen": { "field": "<other-field-name>", "notEquals": <value> } }
```

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `field` | string | Yes | `name` of another prescout field to compare against. Must be declared before this field in the config (preceding fields only). |
| `equals` | integer | One of `equals`/`notEquals` | Show this field only when the referenced field's value equals this integer. |
| `notEquals` | integer | One of `equals`/`notEquals` | Show this field only when the referenced field's value does not equal this integer (and the field has a value). |

**Example:** show a "Climb Height" comment only when the "Climbs?" checkbox is selected:
...
},
{
  ...
  "name": "climb_notes",
  "label": "Climb Notes",
  "showWhen": { "field": "climbs", "equals": 1 }
}
```

**Example:** `hasOther` on a `singleSelect`:
...
Selecting "Other" reveals a text input. The typed value (e.g. `"Kotlin"`) is stored directly as the display value in the `[{field, value}]` array.

#### Form Page — `/scout-leads/prescout/`

- Scouts enter a team number and press **Load** to fetch any existing prescout form data for that team.
- If data exists, the form pre-populates and shows an "Editing existing prescout" indicator.
- Form sections are rendered in order from the config. `singleSelect` fields render as toggle-tile buttons (with an optional "Other" tile when `hasOther: true`); `comment` fields render as textareas (500-char limit); `starRating` fields render as star buttons.
- Fields with a `showWhen` condition are hidden until the condition is satisfied. Changing the controlling field clears any hidden dependent field values.
- Submitting calls `POST /api/prescout/form`. On success, the entry switches to edit mode.
- A **PhotoGallery** is shown below the form sections for robot photo upload/view for the loaded team.
- An **Upload Spreadsheet (Emergency)** link at the top of the page links to `/scout-leads/prescout/upload` for bulk data entry.
- If the active game config has no `prescout` key defined, the form page shows an informational message and still provides the upload link.

#### Data Storage Format

Form submissions are stored as a JSONB array of `{field, value}` pairs using the field `label` as the key (not `name`). `singleSelect` values are stored as their option label string (not the numeric value). `starRating` values are stored as strings.

```json
[
  { "field": "Drive Type", "value": "Swerve" },
  { "field": "Driver Experience", "value": "First year driver, 2 off-season events" },
  { "field": "Overall Impression", "value": "4" }
]
```

This format is identical to the spreadsheet upload format, so `PrescoutSection` renders both sources the same way.

### Prescout Spreadsheet Format and Upload Page

Upload a `.xlsx` file with a sheet named exactly **`Prescout`** (case-sensitive) at `/scout-leads/prescout/upload`. The sheet uses a **transposed** layout:
...
- Auth: **any authenticated user** (standard session token — no admin password required).
...
- **Clear all data**: two-tap confirmation button calls `DELETE /api/prescout?gameName=<name>`, removing all spreadsheet prescout rows for the active game (form data and photos are unaffected).

> **Note:** `/admin/prescout` redirects to `/scout-leads/prescout/upload`. The "Prescout" button in `/admin/games` and the admin nav card also link here for backwards compatibility.

### API Routes

#### Prescout Form Data (config-driven)

| Method | Route | Auth | Description |
|--------|-------|------|-------------|
| `GET` | `/api/prescout/form?team=<num>&gameId=<id>` | Any authenticated user | Returns `{ data, submittedBy, submittedAt, updatedAt }` for a team. `gameId` optional; defaults to active game. Returns `{ data: null }` if no entry exists. |
| `POST` | `/api/prescout/form` | Any authenticated user | Body: `{ teamNumber, gameId?, data: [{field, value}, ...] }`. **Field-level merge**: existing fields not included in `data` are preserved; submitted fields overwrite matching existing fields. This prevents data loss when two scouts edit different fields for the same team concurrently. Returns `{ success: true, teamNumber }`. |

#### Prescout Spreadsheet Data
...
| `DELETE` | `/api/prescout?gameName=<name>` | Admin only | Deletes all spreadsheet prescout rows for a game. |
| `POST` | `/api/prescout/upload` | Any authenticated user | Accepts `multipart/form-data` with `file` (.xlsx) and `gameName`. Returns `{ imported, teams[] }`. |
| `GET` | `/api/prescout/teams?gameName=<name>` | Any authenticated user | Returns `{ teams[] }` — sorted list of team numbers with spreadsheet prescout data for the game. |
...
|------|--------------|--------|
| `/team-view` | `PrescoutSection` shown below scouting stats (form data takes priority over spreadsheet data) | `TaggedPhotoGrid` rows rendered at placements defined in `display.teamView.photoSections` (config-driven) |
| `/compare` | `PrescoutSection` per team at bottom of each column (form data takes priority over spreadsheet data) | Not shown |
...
| `/scout-leads/prescout/` | Config-driven prescout form: load team → fill sections → submit/update; includes PhotoGallery for robot photos | `PhotoGallery` for the loaded team (upload/delete enabled) |
| `/scout-leads/prescout/upload` | Spreadsheet upload (`.xlsx`), view imported teams, clear all | Not shown |
| `/admin/prescout` | Redirects to `/scout-leads/prescout/upload` | Not shown |
...
- Prescout and photo data are scoped per game by table name. Deleting a game from the admin panel drops all three tables (`prescout_<gameName>`, `prescoutform_<gameName>`, `photos_<gameName>`). There is no `game_name` column inside these tables.
- The `prescoutform_<gameName>` table is created lazily by `ensurePrescoutFormTable()` inside the form API route if it does not exist yet, in addition to being created at game-creation time. This makes it safe to use even on games created before the feature was added.
- `/api/prescout/upload` and `/api/prescout/teams` use standard session token auth (`validateAuthToken`), not admin password auth. The page-level admin password gate has been removed.
